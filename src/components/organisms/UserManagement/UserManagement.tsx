...
export default function UserManagement() {
  ...
  const handleCreateOrUpdate = async (formData: any) => {
    try {
      ...
      if (formData.assignedStationId) {
        const station = stations.find((s) => s.id === formData.assignedStationId);
  ...
  const handleResetPassword = async (user: User) => {
    if (confirm(`Bạn có chắc chắn muốn đặt lại mật khẩu cho ${user.email}?\nMật khẩu mới sẽ được tạo ngẫu nhiên.`)) {
       alert(`Đã đặt lại mật khẩu cho ${user.email}. Một email hướng dẫn đã được gửi đi.`);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 leading-tight mb-2 tracking-tight">Quản lý người dùng</h1>
