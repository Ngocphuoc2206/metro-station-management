...
export default function TicketTypeManagement() {
  ...
  const handleCreateOrUpdate = async (formData: any) => {
    try {
      if (editingType) {
        const updated = await ticketTypeApi.updateTicketType(editingType.id, formData);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 leading-tight mb-2 tracking-tight">Quản lý loại vé</h1>
          <nav className="text-sm font-medium text-gray-400 flex items-center gap-2">
            <span className="hover:text-gray-600 transition-colors cursor-pointer">Admin</span>
            <span>›</span>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[400px] p-6 text-center transform transition-all">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">Xác nhận xóa vé</h3>
            <p className="text-gray-500 text-sm mb-6">Bạn có chắc chắn muốn xóa loại vé này? Thao tác này có thể ảnh hưởng đến các giao dịch đã mua.</p>
