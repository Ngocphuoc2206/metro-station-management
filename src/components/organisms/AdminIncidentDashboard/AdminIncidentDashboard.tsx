...
const MOCK_INCIDENTS: IncidentRecord[] = [
  { id: "INC-001", title: "Hỏng màn hình quét vé", stationId: "Ga Bến Thành", deviceId: "GATE-BT-01", deviceType: "gate", severity: "high", status: "Open", assigneeName: undefined, description: "Màn hình cảm ứng tại máy bán vé số 4 không phản hồi thao tác của người dùng, cần kiểm tra kết nối và phần cứng.", createdAt: "2026-06-06T01:00:00Z", updatedAt: "2026-06-06T01:00:00Z" },
  { id: "INC-002", title: "Lỗi thẻ nhớ và màn hình", stationId: "Ga Suối Tiên", deviceId: "GATE-ST-03", deviceType: "gate", severity: "medium", status: "Assigned", assigneeName: "Nguyễn Văn An", description: "Thẻ nhớ bị hỏng, màn hình hiển thị nhiễu.", createdAt: "2026-06-06T01:30:00Z", updatedAt: "2026-06-06T02:00:00Z" },
  { id: "INC-003", title: "Hỏng thẻ nhớ thiết bị", stationId: "Ga Tân Cảng", deviceId: "DEV-TC-07", deviceType: "device", severity: "low", status: "InProgress", assigneeName: "Lê Văn Tuấn", description: "Thẻ nhớ cần thay mới, thiết bị không khởi động được.", createdAt: "2026-06-06T00:00:00Z", updatedAt: "2026-06-06T02:10:00Z" },
  { id: "INC-004", title: "Hỏng màn hình tiền sảnh", stationId: "Ga Nhà hát TP", deviceId: "DISP-NH-02", deviceType: "display", severity: "critical", status: "InProgress", assigneeName: "Trần Minh", description: "Màn hình tắt đột ngột, không hiển thị thông tin tàu.", createdAt: "2026-06-05T22:00:00Z", updatedAt: "2026-06-06T01:55:00Z" },
  { id: "INC-005", title: "Lỗi quét thẻ thanh toán", stationId: "Ga Bình Thái", deviceId: "GATE-BT-09", deviceType: "gate", severity: "high", status: "Resolved", assigneeName: "Nguyễn Văn An", description: "Cổng quét không nhận thẻ từ, hành khách không qua được.", createdAt: "2026-06-05T18:00:00Z", updatedAt: "2026-06-06T00:30:00Z" },
  { id: "INC-006", title: "Hỏng máy in vé tự động", stationId: "Ga Bà Chiểu", deviceId: "PRINT-BC-01", deviceType: "printer", severity: "medium", status: "Resolved", assigneeName: "Lê Văn Tuấn", description: "Máy in hết giấy và bị kẹt giấy, không in được vé.", createdAt: "2026-06-05T16:00:00Z", updatedAt: "2026-06-05T20:00:00Z" },
  { id: "INC-007", title: "Camera an ninh offline", stationId: "Ga Phước Long", deviceId: "CAM-PL-04", deviceType: "camera", severity: "high", status: "Open", assigneeName: undefined, description: "Camera không kết nối được tới server ghi hình trung tâm.", createdAt: "2026-06-06T02:00:00Z", updatedAt: "2026-06-06T02:00:00Z" },
  { id: "INC-008", title: "Thang cuốn dừng hoạt động", stationId: "Ga An Phú", deviceId: "ESC-AP-02", deviceType: "elevator", severity: "critical", status: "Open", assigneeName: undefined, description: "Thang cuốn B2 bị kẹt cơ học, cần kỹ thuật viên kiểm tra ngay.", createdAt: "2026-06-06T01:45:00Z", updatedAt: "2026-06-06T01:45:00Z" },
  { id: "INC-009", title: "Lỗi hệ thống bảng giờ", stationId: "Ga Rạch Chiếc", deviceId: "SYS-RC-01", deviceType: "system", severity: "low", status: "Resolved", assigneeName: "Trần Minh", description: "Bảng giờ tàu hiển thị sai lệch 10 phút, cần đồng bộ lại.", createdAt: "2026-06-05T14:00:00Z", updatedAt: "2026-06-05T17:00:00Z" },
  { id: "INC-010", title: "Mất điện tạm thời khu vực A", stationId: "Ga Thủ Đức", deviceId: "ELEC-TD-01", deviceType: "electric", severity: "critical", status: "InProgress", assigneeName: "Nguyễn Văn An", description: "Mất điện khu A, hệ thống UPS đang hoạt động, cần khôi phục điện lưới.", createdAt: "2026-06-06T00:30:00Z", updatedAt: "2026-06-06T01:30:00Z" },
];
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; dot: string; bg: string }> = {
    Open: { label: "Tạo mới", dot: "bg-gray-400", bg: "bg-gray-100 text-gray-600 border border-gray-200" },
    Assigned: { label: "Đã phân công", dot: "bg-blue-500", bg: "bg-blue-50 text-blue-700 border border-blue-200" },
    InProgress: { label: "Đang xử lý", dot: "bg-orange-500", bg: "bg-orange-50 text-orange-700 border border-orange-200" },
    Escalated: { label: "Đang xử lý", dot: "bg-orange-500", bg: "bg-orange-50 text-orange-700 border border-orange-200" },
    Resolved: { label: "Đã hoàn thành", dot: "bg-green-500", bg: "bg-green-50 text-green-700 border border-green-200" },
    Closed: { label: "Đã đóng", dot: "bg-slate-500", bg: "bg-slate-100 text-slate-600 border border-slate-200" },
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl flex flex-col">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Mã sự cố</p>
            ...
            <div className="col-span-2">
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Tên sự cố</p>
              <p className="font-semibold text-gray-900">{incident.title}</p>
            </div>
            <div className="col-span-2">
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Mô tả chi tiết</p>
              <p className="text-gray-700 text-sm leading-relaxed">{incident.description || "Không có mô tả"}</p>
            </div>
            <div>
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Mức độ</p>
              <SeverityBadge severity={incident.severity} />
            </div>
            <div>
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Nhà ga</p>
            ...
            <div>
              ...
              {selectedStaffId && (
                <div className="mt-2 flex items-center gap-2 text-xs text-green-700 bg-green-50 rounded-lg px-3 py-2">
                  ...
                  Sẽ phân công cho: <span className="font-semibold">{MOCK_STAFF.find(s => s.id === selectedStaffId)?.name}</span>
                </div>
              )}
            </div>
              ...
              <div className="space-y-2">
                ...
                {evidenceImages.map((src, idx) => (
                  <div key={idx} className="rounded-xl overflow-hidden border border-gray-100 shadow-sm">
                    ...
                    <img src={src} alt={`Bằng chứng ${idx + 1}`} className="w-full object-cover max-h-52" />
                  </div>
                ))}
              </div>
            </>
...
export default function AdminIncidentDashboard() {
  ...
  const stats = useMemo(() => ({
    ...
    inProgress: incidents.filter((i) => ["InProgress", "Escalated", "Assigned"].includes(i.status)).length,
  ...
  const filtered = useMemo(() => {
    ...
    else if (statusFilter === "active") list = list.filter((i) => ["InProgress", "Assigned", "Escalated"].includes(i.status));
    else if (statusFilter === "done") list = list.filter((i) => ["Resolved", "Closed"].includes(i.status));
    if (searchQuery.trim()) {
  ...
  const handleSaved = (id: string, assigneeName: string, status: string) => {
    setIncidents((prev) =>
      prev.map((inc) =>
        inc.id === id ? { ...inc, assigneeName: assigneeName || inc.assigneeName, status: status as any } : inc,
      ),
    );
    // Cập nhật selectedIncident nếu đang mở
    setSelectedIncident((prev) =>
      prev?.id === id ? { ...prev, assigneeName: assigneeName || prev.assigneeName, status: status as any } : prev,
    );
  };
  ...
  return (
    <div className="flex flex-col space-y-6 w-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Tổng sự cố", value: stats.total, colorBase: "blue", icon: "M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" },
          { label: "Mới mở", value: stats.pending, colorBase: "gray", icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" },
          { label: "Đang xử lý", value: stats.inProgress, colorBase: "orange", icon: "M13 10V3L4 14h7v7l9-11h-7z" },
          { label: "Đã xử lý", value: stats.resolved, colorBase: "green", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-gray-100 p-5 flex items-center gap-4 shadow-sm">
            <div className={`w-10 h-10 rounded-full bg-${s.colorBase}-50 flex items-center justify-center shrink-0`}>
              <svg className={`w-5 h-5 text-${s.colorBase}-500`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex items-center gap-1 bg-gray-50 p-1 rounded-xl">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all whitespace-nowrap ${statusFilter === f.value ? "bg-white text-blue-600 shadow-sm" : "text-gray-500 hover:text-gray-700"
                  }`}
              >
                {f.label}
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${statusFilter === f.value ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-500"
                  }`}>
                  {f.count}
                </span>
              </button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-50">
                {["Mã SC", "Tên sự cố", "Nhà ga", "Mức độ", "Nhân viên", "Trạng thái", "Thao tác"].map((h) => (
                  ...
                  : filtered.map((inc) => (
                    <tr key={inc.id} className="hover:bg-blue-50/30 transition-colors">
                      <td className="px-5 py-4">
                        <span className="font-bold text-blue-600">{toShortCode(inc.id)}</span>
                      </td>
                      <td className="px-4 py-4 max-w-[200px]">
                        <span className="font-semibold text-gray-900 truncate block">{inc.title}</span>
                      </td>
                      <td className="px-4 py-4 text-gray-500 text-xs">{inc.stationId || "—"}</td>
