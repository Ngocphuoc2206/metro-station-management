...
const schema = z.object({
  ...
  validityDuration: z.coerce.number().min(1, "Thời gian hiệu lực phải lớn hơn 0"),
...
export default function TicketTypeFormModal({ isOpen, onClose, ticketType, onSubmit }: Props) {
  const {
    register,
    handleSubmit,
    reset,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[600px] overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 flex items-center justify-between border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900">{isEdit ? "Cập nhật loại vé" : "Thêm loại vé mới"}</h2>
        ...
        <div className="px-6 py-6 overflow-y-auto w-full custom-scrollbar">
          <form id="ticket-type-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            
            {/* ROW 1: Tên loại vé + Mã loại */}
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-blue-900 mb-1.5">Tên loại vé</label>
                <input
                  type="text"
                  {...register("name")}
              ...
              <div>
                <label className="block text-xs font-bold text-blue-900 mb-1.5">Mã loại</label>
                <input
                  type="text"
                  {...register("code")}
                  className={`w-full px-4 py-2.5 bg-gray-50 border rounded-xl text-sm font-medium uppercase focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 transition-colors ${
                    errors.code ? "border-red-300" : "border-gray-200 focus:border-blue-400"
                  }`}
            ...
            <div>
              <label className="block text-xs font-bold text-blue-900 mb-1.5">Thời gian hiệu lực</label>
              <div className="flex gap-3">
                <div className="flex-1">
                  <input
                    ...
                    className={`w-full px-4 py-2.5 bg-gray-50 border rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 transition-colors ${
                      errors.validityDuration ? "border-red-300" : "border-gray-200 focus:border-blue-400"
                    }`}
                    placeholder="Số lượng"
                  />
                  {errors.validityDuration && <p className="text-red-500 text-xs mt-1">{errors.validityDuration.message}</p>}
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-blue-900 mb-1.5">Giá niêm yết</label>
                <div className="relative">
                  <input
                    ...
                    className={`w-full pl-4 pr-12 py-2.5 bg-gray-50 border rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100 transition-colors ${
                      errors.price ? "border-red-300" : "border-gray-200 focus:border-blue-400"
                    }`}
                    placeholder="0"
                  />
              ...
              <div>
                <label className="block text-xs font-bold text-blue-900 mb-2">Trạng thái</label>
            ...
            <div>
              <label className="block text-xs font-bold text-blue-900 mb-1.5">Quy định sử dụng</label>
              <textarea
                {...register("conditions")}
                rows={4}
