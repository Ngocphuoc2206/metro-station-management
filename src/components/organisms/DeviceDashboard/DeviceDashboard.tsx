...
import { MOCK_DEVICES } from "@features/device/deviceMockData";
  ...
  const filtered = useMemo(() => {
    return MOCK_DEVICES.filter((d) => {
      ...
      if (search) {
        ...
        if (!d.id.toLowerCase().includes(q) && !d.station.toLowerCase().includes(q) && !d.model.toLowerCase().includes(q)) return false;
      }
      return true;
    });
        ...
        <div className="flex gap-0 border-b border-gray-100 mb-5">
          {TABS.map((tab) => {
            ...
            return (
              <button
                key={tab.value}
                onClick={() => { setActiveTab(tab.value); setSelectedDevice(null); setSearch(""); setStatusFilter("all"); }}
                className={`relative px-4 py-3 text-sm font-medium transition-colors flex items-center gap-2 ${
                  active ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:text-gray-800"
                }`}
              >
                {tab.label}
