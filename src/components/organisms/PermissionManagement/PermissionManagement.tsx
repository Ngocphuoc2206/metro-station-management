...
export default function PermissionManagement() {
  ...
  const hasChanges = useMemo(() => {
    if (!selectedRoleOriginal || !localPermissions[selectedRoleOriginal.id]) return false;
  ...
  const handleSaveRole = async (roleId: string) => {
    ...
    try {
      ...
      const updatedRole = await permissionApi.updateRolePermissions(roleId as any, newPermissionsArray);
      
      // Cập nhật lại mảng roles gốc để tắt nút Save (vì đã đồng bộ)
      setRoles(prev => prev.map(r => r.id === roleId ? updatedRole : r));
      
      showToast(`Đã cập nhật phân quyền hệ thống cho ${updatedRole.name} thành công.`, "success");
  ...
  return (
    <div className="flex flex-col gap-6 relative min-h-[calc(100vh-6rem)]">
      ...
      <div className="block lg:hidden bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-xl text-sm font-medium">
        Bạn nên xoay ngang màn hình hoặc xem trên Máy tính để Hiển thị Ma trận rõ ràng nhất.
      </div>

      <div className="flex flex-col items-start gap-1">
        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Phân quyền & Vai trò</h1>
        <p className="text-sm text-gray-500">Thiết lập giới hạn truy cập và xem chi tiết theo nhóm người dùng hệ thống.</p>
      </div>
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-5 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-blue-800/80 leading-relaxed">
            Tất cả các vai trò hiển thị theo dạng ma trận để dễ dàng so sánh. Cột 'Quản trị viên' luôn giữ mức ưu tiên tối đa. Quản trị hệ thống vui lòng cân nhắc thận trọng khi cấp quyền mới cho các nhóm đặc thù. Click vào Tiêu đề Cột của từng vai trò để mở tuỳ chỉnh lưu lại thay đổi.
          </p>
        </div>
      </div>
      ...
      {/* TOAST THÔNG BÁO */}
      <div className={`fixed top-4 right-4 z-[60] transition-all duration-300 transform ${toast ? "translate-y-0 opacity-100" : "-translate-y-4 opacity-0 pointer-events-none"}`}>
        {toast && (
          <div className="bg-white rounded-xl shadow-2xl border border-gray-100 p-4 flex gap-3 min-w-[320px] animate-in fade-in slide-in-from-top-2">
            ...
            <div>
              <h4 className={`text-sm font-bold mb-0.5 ${toast.type === "success" ? "text-gray-900" : "text-red-600"}`}>
                {toast.type === "success" ? "Thành công" : "Thất bại"}
              </h4>
              <p className="text-xs text-gray-500 leading-tight">{toast.message}</p>
            </div>
            <button onClick={() => setToast(null)} className="ml-auto text-gray-400 hover:text-gray-600 p-1">
